USERS = [Admin.active, Trucker.active, Customer.active.map{|c| c.employees.active.first }.compact].map{|users| users.sort_by(&:name_with_role) }.flatten rescue []
...
SwitchUser.setup do |config|
  ...
  config.available_users = { user: ->{ USERS } }
  ...
  config.available_users_names = { :user => :name_with_role }

  # controller_guard is a block,
  # if it returns true, the request will continue,
  # else the request will be refused and returns "Permission Denied"
  # if you switch from "admin" to user, the current_user param is "admin"
  config.controller_guard = lambda { |current_user, request| !Rails.env.production? }
