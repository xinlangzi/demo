class Shipper < CustomersClient
  ...
  def self.states(customer_id=nil)
    ...
    ids = Shipper.search(options).result.reorder('').select(:address_state_id).distinct
  ...
  def self.cities(state_id=nil, customer_id=nil)
    ...
    cities = Shipper.search(options).result.reorder('').select(:address_city).distinct
